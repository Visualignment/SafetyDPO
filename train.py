# This script is modified based on https://github.com/SalesforceAIResearch/DiffusionDPO.git
#!/usr/bin/env python
# coding=utf-8
...
for i in range(torch.cuda.device_count()):
   print('GPUS', torch.cuda.get_device_properties(i).name, '\n\n\n\n')
...
DATASET_NAME_MAPPING = {
    ...
    "./data/dpo_data": ("jpg_0", "jpg_1", "label_0", "caption"),
    "./data/simple": ("jpg_0", "jpg_1", "label_0", "caption"),
}
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        dataset_columns = DATASET_NAME_MAPPING.get("./data/dpo_data", None)
    ...
    if args.train_method == 'dpo':
        ...
        def preprocess_train(examples):
            all_pixel_values = []
            if 'pickapic_formatted' in args.dataset_name:
                for col_name in ['jpg_0', 'jpg_1']:
                    images = [Image.open(os.path.join(args.dataset_name, paths)).convert("RGB")
                                for paths in examples[col_name]]
    ...
    #pytorch_total_params = sum(p.numel() for p in lora_layers)
